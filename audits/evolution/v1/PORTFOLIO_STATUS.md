# Formal-science portfolio status after evolution v1

Date: 21 September 2026. This ledger accompanies the evolution audit for source commit `a60a92b6064b4dde33a98d3c79be195c77595873`. It is a reconciliation of the recorded portfolio with the accepted local work, not a fresh global search or a claim of novel mathematics.
...
| 10 | GKSL characterization | C: QICLean finite-dimensional iff source candidate | Explicit dissipator, two-state generator, and all-matrix flow with semigroup and matrix-valued ODE identities; no complete positivity or generic GKSL characterization yet |
| 11 | Quantum Perron-Frobenius | C: positive-map/irreducibility/primitivity machinery | Specific two-state stationary uniqueness and an explicit flow fixing stationary matrices; no convergence/attractivity or generic PF theorem yet |
| 12 | Lieb-Robinson bounds | U/P: endpoint not established | None |
| 13 | State layer, partial trace, purification | C: several downstream implementations | Qubit density predicate, basis densities, diagonal-state validity; no partial trace/purification |
| 14 | POVMs/Born rule | C: downstream infrastructure | None |
| 15 | Kraus/Choi/Stinespring equivalences | C: downstream candidates | Finite Kraus trace identity/preservation and an explicit trace-preserving evolution; no evolution Kraus certificate, CP theorem, or representation equivalences yet |
...
After acceptance of the evolution milestone, project progress is **four accepted bounded increments** (Stage 0, dissipator, stationary, evolution), involving 115 exports: 100 theorem consumer contracts and 15 definitions/abbreviations. The evolution increment adds 43 exports: 39 theorems and four definitions. These counts are verification coverage, not mathematical novelty or percentage of the portfolio complete. None of #10, #11, #13, or #15 is closed at its full inventory scope by this pilot.
...
The next bounded task is `kraus`: construct the four explicit Kraus matrices for the physical parameter range, prove normalization and equality with the accepted evolution, and establish complete positivity with a bridge that quantifies over every finite ancilla. The current assignment is issued with this audit as `NEXT_FABLE_TASK.md`; the root copy becomes the mutable active slot after the immutable issued copy is stored under `audits/evolution/v1/`.

The current evolution proofs concern all real rates and all real times as mathematical statements. Trace and Hermiticity preservation do not establish positivity or complete positivity. The upcoming channel interpretation restricts to nonnegative rates and nonnegative time and must cover its physical boundary cases explicitly.

Quantitative convergence in a named Frobenius norm remains a later checkpoint. The original roadmap grouped explicit evolution, channels, and convergence under Stage 3; separating them keeps the implementation and audit boundary concrete. Neither complete positivity nor convergence is credited in this ledger.
...
- `evidence/evolution/v1/verification/`: submitted final evolution verification records. Earlier milestone evidence remains preserved at its recorded paths.
- `audits/stage0/v1/`, `audits/dissipator/v1/`, `audits/stationary/v1/`, and this returned `audits/evolution/v1/` report: audit decisions and their stated limits.
