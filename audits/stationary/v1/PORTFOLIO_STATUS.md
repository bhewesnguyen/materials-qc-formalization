# Formal-science portfolio status after stationary v1

Date: 20 September 2026. This ledger accompanies the stationary audit. It is a reconciliation of the recorded portfolio with the accepted local work, not a fresh global search or a claim of novel mathematics.
...
| 10 | GKSL characterization | C: QICLean finite-dimensional iff source candidate | Explicit dissipator and two-state generator only; no generic GKSL characterization or CP semigroup yet |
| 11 | Quantum Perron-Frobenius | C: positive-map/irreducibility/primitivity machinery | Specific two-state stationary uniqueness; no generic PF theorem or attractivity yet |
...
| 15 | Kraus/Choi/Stinespring equivalences | C: downstream candidates | Finite Kraus trace identity/preservation; no CP theorem or representation equivalences |
...
After acceptance of the stationary milestone, project progress is **three accepted bounded increments** (Stage 0, dissipator, stationary), involving 72 exports and 61 theorem consumer contracts. The stationary increment adds 30 exports and 28 contracts. These counts are verification coverage, not mathematical novelty or percentage of the portfolio complete. None of #10, #11, #13, or #15 is closed at its full inventory scope by this pilot.
...
The next bounded task is `evolution`: a transparent complex-linear flow on every complex qubit matrix, its semigroup and real-line derivative identities, and exact zero-total-rate behavior. The current assignment is issued with this audit as `NEXT_FABLE_TASK.md`; the root copy becomes the mutable active slot after the immutable issued copy is stored under `audits/stationary/v1/`.

After its audit, the remaining dynamics checkpoints are finite Kraus certification with a valid complete-positivity bridge, then quantitative convergence in a named Frobenius norm. The original roadmap grouped these under Stage 3; separating them keeps the implementation and audit boundary concrete. None of these later results is credited in this ledger.
...
- `evidence/stationary/v1/verification/`: submitted final verification records.
- `audits/stage0/v1/`, `audits/dissipator/v1/`, and this returned `audits/stationary/v1/` report: audit decisions and their stated limits.
